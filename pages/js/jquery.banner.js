$(function() {
    ...
    $.fn.jQBanner = function(defaults){
		...
		config.objArw = "#"+this.attr("id")+" .clsBannerArrow";
    ...
    function jsBanInit(config) {
		...
		$(config.objArw+" img").each(function(){ $(this).css("cursor","pointer") });
    ...
    function jsBanBtnBind(config) {
        ...
        if (config.isBtnAct=="mouseover") {
			...
			$(config.objArw+" img").hover(function()
			{
				clearTimeout(config.objTimer);
			},
			function()
			...
			$(config.objArw+" img.left").click(function()
			{				
				config.nOrderNoBtn = config.nOrderNo - 1;
				if(config.nOrderNoBtn < 0) config.nOrderNoBtn = config.nCount - 1;
				jsBanAction(config);
			});

			$(config.objArw+" img.right").click(function()
			{
				config.nOrderNoBtn = config.nOrderNo + 1;
				if(config.nOrderNoBtn > config.nCount - 1) config.nOrderNoBtn = 0;
				jsBanAction(config);
			});
