...
//페이징
function getPageLink($lnum,$p,$tpage,$img,$arr)
{
	include_once $GLOBALS['g']['path_core'].'function/lib/page.lib.php';
	return LIB_getPageLink($lnum,$p,$tpage,$img,$arr);
}
//문자열끊기
...
function getUploadImage2($upfiles,$d,$content,$ext)
{
	include_once $GLOBALS['g']['path_core'].'function/lib/getUploadImage.lib.php';
	return LIB_getUploadImage2($upfiles,$d,$content,$ext);
}
function getUploadImage_src($upfiles,$d,$content,$ext)
{
	include_once $GLOBALS['g']['path_core'].'function/lib/getUploadImage_src.lib.php';
	return LIB_getUploadImage_src($upfiles,$d,$content,$ext);
}
//도메인
...
function getSendSMS($frame,$form,$cpnumber,$title,$content,$xms="sms")
{
	...
	echo "	<input type='hidden' name='title' value='".urlencode(iconv('utf-8','euc-kr',$title))."' />";
	echo "	<input type='hidden' name='cpnumber' value='".str_replace('-','',$cpnumber)."' />";
	echo "	<input type='hidden' name='content' value='".urlencode(iconv('utf-8','euc-kr',$content))."' />";	
	echo "</form>";
	echo "";
