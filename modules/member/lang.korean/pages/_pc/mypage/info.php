...
<div id="pages_join">
	...
	<td>
		<input type="text" name="name" value="<?php echo $my['name']?>" maxlength="10" class="input" readonly="readonly" />
	</td>
	</tr>
	...
	<?php $SNS=explode('|',$my['sns'])?>
	<?php $SNSSET=array('트위터','페이스북','미투데이','요즘')?>
	<tr>
	<td class="key">SNS 아이디</td>
	<td>
		<?php for($i = 0; $i < 4; $i++):?>
		<input type="text" name="sns_<?php echo $i?>" value="<?php echo $SNS[$i]?>" class="input" /> - <?php echo $SNSSET[$i]?><br /> 
		<?php endfor?>
	</td>
	</tr>
	<?php endif?>
