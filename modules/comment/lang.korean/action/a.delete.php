...
$nLocation1 = getLinkFilter($g['s'].'/?'.($_HS['usescode']?'r='.$r.'&amp;':'').($c?'c='.$c:'m='.$m),array('skin','iframe','sort','orderby','recnum','where','keyword'));
$nLocation2 = $nLocation1.'&uid='.$R['uid'];

if ($my['uid'] != $R['mbruid'] && !$my['admin'])
{
	if ($pw)
	{
		if (md5($pw) != $R['pw']) getLink($nLocation2,'parent.','비밀번호가 일치하지 않습니다.','');
	}
	else {
		 getLink($nLocation2,'parent.','비밀번호를 입력해 주세요.','');
	}
}
