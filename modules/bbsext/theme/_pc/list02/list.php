<?php
	...
	## 현재 주소 저장
	$strTemp = $_SERVER['REQUEST_URI'];
	$strTemp = base64_encode($strTemp);
	$_SESSION['LIST_URL'] = $strTemp;
?>
		...
		<div class="category" style="width:960px;border:0px solid">
			<ul id="gnb">
				...
				<li><select id="tmp_reception">
						...
						<option value="BC카드"<? if( $reception == "BC카드" ) echo "selected"?> >BC카드</option>
					</select>
				</li>
				<li><select id="tmp_admin_email">
						...
						<option value="<?php echo $row['name'];?>"<?php if($selected_admin==$row['name'] || $admin_email==$row['name']){echo " selected";}?>><?php echo $row['name'];?></option>
						<?php endwhile;?>
						<?php endif;?>
					</select>
				</li>
