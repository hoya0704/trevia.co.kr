...
function getSendMail($to,$from,$subject,$content,$html) 
{
	...
	return @mail($To,getUTFtoKR($subject),getUTFtoKR($content),$Header,"-f $Frm");
}
...
if($db_host&&$db_id&&$db_passwd&&$db_name) {
  ...
  if($conn) if(!@mysql_select_db($db_name,$conn)) error_page("Databases Name is not correct");
	...
	for($i=0;$i<10;$i++):
		...
		else if ($i!=0) $condition_air .= "||";

		$condition_air .="{$_POST['nadd'.$k++]}||{$_POST['nadd'.$k++]}||{$_POST['nadd'.$k++]}||{$_POST['nadd'.$k++]}||{$_POST['nadd'.$k++]}||{$_POST['nadd'.$k++]}||{$_POST['nadd'.$k++]}||{$_POST['nadd'.$k++]}||";	
		$k++;
	endfor;

	$condition_air = getUTFtoKR($condition_air);

	$k=0;
	$condition_hotel = "";
	for($i=0;$i<10;$i++):
		if(!$_POST['madd'.$k]){break;}
		else if ($i!=0) $condition_hotel .= "||";

		$condition_hotel .= "{$_POST['madd'.$k++]}||{$_POST['madd'.$k++]}||{$_POST['madd'.$k++]}||{$_POST['madd'.$k++]}||{$_POST['madd'.$k++]}||{$_POST['madddata'.$i]}";
		$k++;
	endfor;

	$condition_hotel = getUTFtoKR($condition_hotel);

	$supply = getUTFtoKR("{$_POST[new_val30]}||{$_POST[new_val31]}||{$_POST[val44]}||{$_POST[val45]}||{$_POST[val46]}||{$_POST[val47]}||{$_POST[new_val32]}");

	$tel = getUTFtoKR("{$_POST[val48]}||{$_POST[val49]}||{$_POST[val50]}||{$_POST[val51]}||{$_POST[val52]}||{$_POST[val53]}||{$_POST[val54]}||{$_POST[val55]}||{$_POST[val56]}");

	$email = $_POST[to_email];

	$qrys = "insert into  rb_contract_data values ( NULL, {$puid} , '{$subject}', '{$name}', '{$period}', '{$cost}', '{$condition_air}', '{$condition_hotel}', '{$supply}', '{$tel}', '{$email}', '{$ndate}',NUlL,'','')";
...
<script type='text/javascript' charset='utf-8' src='http://trevia.co.kr/mail/script.js'></script>
  ...
  <tr>
	...
	<table width='800' hight='200' border='0' cellspacing='1' cellpadding='0' bgcolor='#545350'>
      ...
      <tr>
        ...
        <td bgcolor='white' align='center'>".$_POST[val8_1]."</td>
      </tr>";
?>
<?if($_POST[val9] != '') {?>
<?
  ...
  <tr>
    <td><table width='800' border='0' cellspacing='1' cellpadding='0' bgcolor='#545350'>
      ...
      <tr>
        ...
        <td bgcolor='#ffffff' align='center'>".$_POST[val32]."</td>
      </tr>
      <tr>
        ...
        <td bgcolor='#ffffff' align='center'>".$_POST[val33]."</td>
      </tr>
      <tr>
        ...
        <td bgcolor='#ffffff' align='center'>".$_POST[val34]."</td>
      </tr>
      <tr>
        ...
        <td bgcolor='#ffffff' align='center'>".$_POST[val35]."</td>
      </tr>
      <tr>
        <td bgcolor='#E6E6E6' align='center'><span class='style4'><strong>적용환율</strong></span></td>
        <td height='20' colspan='1' align='center' bgcolor='#ffffff'><strong>".$_POST[new_val29]."</strong></td>
        ...
        <td bgcolor='#ffffff' align='center'>".$_POST[val36]."</td>
      </tr>
      <tr>
        <td bgcolor='#E6E6E6' align='center'><span class='style4'><strong>예상 총금액</strong></span></td>
        <td height='20'  align='center' bgcolor='#ffffff'><span class='style15'>".$_POST[ext1]."</span></td>
        <td bgcolor='#f1f1f1' align='center'><strong>원화총액(KRW)</strong></td>
        <td bgcolor='#ffffff' align='center'><strong>".$_POST[ext2]."</strong></td>
        <td bgcolor='#f1f1f1' align='center'><strong>달러총액(".$_POST[ext4].")</strong></td>
        <td bgcolor='#ffffff' align='center'><strong>".$_POST[ext3]."</strong></td>
        <td bgcolor='#ffffff' align='center'></td>
      </tr>
	  <tr>
		<td bgcolor='#E6E6E6' align='center'><span class='style4'><strong>비 고</strong></span></td>
		<td colspan='6'  bgcolor='#ffffff'>&nbsp;".$_POST[val_24]."</td>
	  </tr>
    </table></td>
  </tr>
<tr>
...
<? 	$content .= "<table width='800' border='0' cellspacing='1' cellpadding='0' bgcolor='#545350'>
      <tr>
        <td colspan='8' rowspan='1' align='center' bgcolor='#E6E6E6'><span class='style4'><b>항공여정 예약정보</b></span></td>
	  </tr>
	  <tr>
      ...
      <tr>
        <td width='150' height='20' align='center' bgcolor='#f1f1f1'><span class='style4'><b>여정</b></span></td>
        ...
        <td width='150' bgcolor='#f1f1f1' align='center'><span class='style4'><b>여정</b></span></td>
        <td width='85' bgcolor='#f1f1f1' align='center'><span class='style4'><b>편명</b></span></td>
...
<?
	...
	<table width='800' border='0' cellspacing='1' cellpadding='0' bgcolor='#545350'>
      <tr>
        <td colspan='7' rowspan='1' align='center' bgcolor='#E6E6E6'><span class='style4'><b>숙박시설 예약정보</b></span></td>
	  </tr>
	  <tr>
...
<?
	$s_date = $_POST['madddata'.$i];
	$temp_date = $k + 2;
	$plus_date = $_POST['madd'.$temp_date];
	$check_out = $s_date?date("Y-m-d", strtotime($s_date." ". $plus_date." day")):'';
	$content .= "
	<tr>
	 <td align='center' bgcolor='#ffffff'>".$_POST['madd'.$k++]."</td>
	<td align='center' bgcolor='#ffffff'>".$_POST['madd'.$k++]."</td>
	<td width='80' bgcolor='#ffffff' align='center'>".$_POST['madddata'.$i]."</td>
	<td width='80' bgcolor='#ffffff' align='center'>".$_POST['madd'.$k++]."</td>
	<td width='80' bgcolor='#ffffff' align='center'>".$check_out."</td>
	<td width='80' bgcolor='#ffffff' align='center'>".$_POST['madd'.$k++]."</td>
	<td align='center' bgcolor='#ffffff'>".$_POST['madd'.$k++]."</td>
	</tr>
	";
...
$sql = mysql_query("SELECT * FROM rb_s_mbrdata where name = '".getUTFtoKR($_POST[val48])."' and admin=1;");
$man = mysql_fetch_array($sql);
  ...
  <tr>
   ...
   <td><table width='800' border='0' cellpadding='0' cellspacing='1' bgcolor='#545350' >
      <tr>
        <td width='105' rowspan='2' align='center' bgcolor='#E6E6E6'><span class='style4'>항목</span></td>
      ...
      <tr>
        ...
        <td width='80' height='20' align='center' bgcolor='#ffffff'><span class='style4'>최고2억원</span></td>
        <td width='80' bgcolor='#ffffff' align='center'><span class='style4'>최고2억원</span></td>
        ...
        <td width='80' bgcolor='#ffffff' align='center'><span class='style4'>최고2천만원</span></td>
        <td width='80' bgcolor='#ffffff' align='center'><span class='style4'>최고1천만원</span></td>
        <td width='80' align='center' bgcolor='#ffffff'><span class='style4'>최고50만원</span></td>
      ...
      <tr>
        ...
        <td align='center' bgcolor='#ffffff'><span class='style4'>최고50만원</span></td>
...
영업보증보험 [금액 : 2 억원] / 피보험자 : 주)트레비아</div></td>
	...
	<tr>
        <td colspan='1' align='center' bgcolor='#ffffff'><span class='style4'>비고</span></td>
        <td colspan='9' align='center' bgcolor='#f1f1f1'><span class='style4'><b>상기 여행자보험 보장내역은 LIG 해외여행보험기준입니다. 기타 여행자보험 상품 가입 시 보장내역은 상품에 따라 상이할 수 있습니다.
        </b></span></td>
      </tr>        
    </table></td>
  </tr>
  ...
  <table  width='800' height='50' border='0' cellpadding='0' cellspacing='1' bgcolor='#545350' >
      <tr>
		...
		<td align='center' bgcolor='#F1F1F1'><span class='style4'> D-45일</span></td>
  ...
  <tr>
    <td><table width='800' border='0' cellpadding='0' cellspacing='1' bgcolor='#545350'>
      ...
      <tr>
        ...
        <td align='center' bgcolor='#ffffff'>".$man[19]."</td>
      </tr>
    </table></td>
  </tr>
  <tr>
  ...
  <tr>
    <td><table width='800' border='0' cellspacing='1' cellpadding='0' bgcolor='#545350'>
      <tr>
        ...
        <td width='210' height='25' align='center' bgcolor='#ffffff'>".$_POST[val51]."</td>
        <td width='140' align='center' bgcolor='#ffffff'><span class='style4'>".$_POST[val52]."</span></td>
        <td width='100' rowspan='1'  align='center' bgcolor='#ffffff'>".$_POST[val53]."</td>
        <td width='169' align='center' bgcolor='#ffffff'><span class='style4'>".$_POST[val53_1]."</span></td>
      </tr>
      <tr>
        ...
        <td width='100' rowspan='1'  align='center' bgcolor='#ffffff'>".$_POST[val56]."</td>
        <td align='center' bgcolor='#ffffff'>".$_POST[val56_1]."</td>
      </tr>
      <tr>
...
② 특수지역에의 여행으로서 정당한 사유가 있는 경우에는 당사의 여행약관의 내용과 달리 정할 수 있습니다.
  ...
  <tr>
    <td height='20'><span class='style4'><strong>[ 특별 여행약관 : " . $_POST[add2_2] . " ] </strong></span></td>
  </tr>
  <tr>
    <td>
	</td>
...
제3조 (여행계약 및 여행비용의 납입)
1. 본 상품은 " . $_POST[add2_2] . " 여행 상품으로 리조트와 항공권 등이 포함된 상품입니다.
...
2. 주식회사 트레비아에서는 최대 보험금 1억원의 여행자 보험을 가입해 드립니다. 여행 도중에 우연하게 현지에서 발생하는 불의의 사고 및 질병이 발생할 경우에는 반드시 현지 가이드 및 회사 긴급 연락처에 연락하여 도움을 받으시길 바랍니다. 보험 약관상 현지 병원 진단서 및 영수증, 경찰의 사건 확인서를 받아 오셔야 보험 혜택이 있으므로 가이드와 상의 바랍니다. 보험 약관에 관한 자세한 혜택 내용은 홈페이지(www.samsungfire.co.kr)를 통하여 확인하여 주시기 바랍니다.
  ...
  <tr>
    <td height='40' bgcolor='#e6e6e6'>
		...
		<div align='center' class='style4'><strong>계약서 승인 회신 </strong></div>
	<a href='http://trevia.co.kr/modules/bbsext/theme/_pc/list02/return.php?uid=".$uid."&puid=".$puid."'></a>
	</td>
  </tr>
  <tr>
	<td>&nbsp;</td>
    ...
    <td height='20'>
      <div align='right'>
          <b><font size='3' color='red'>
				<a href='http://trevia.co.kr/modules/bbsext/theme/_pc/list02/return.php?uid=".$uid."&puid=".$puid."'><input type='button' name='sending' value=' 여행 계약 승인 ' style='width:200px;height:40px' /></a>
			</font></b>
		  </div>
		  </form>
	</td>
...
<?// echo getUTFtoKR($_POST[to_email].'|'.$_POST[to_name] . $_POST[admin_add_email].'|'.$_POST[from_name]. $_POST[to_name].'님의 '.$_POST[val3].' 여행계약서 입니다. [주식회사 트레비아, 하이몰디브]')?>

<?
//exit;
//		getSendMail('jacos15@hanmail.net|'.$_POST[to_name] , $_POST[admin_add_email].'|'.$_POST[from_name], $_POST[to_name]."님의 ".$_POST[val3]." 여행계약서 입니다. [주식회사 트레비아, 하이몰디브]", $content, 'HTML');
		getSendMail('jacos15@hanmail.net|'.$_POST[to_name] , $_POST[admin_add_email].'|'.$_POST[from_name], $_POST[to_name]."님의 ".$_POST[val3]." 여행계약서 입니다. [주식회사 트레비아, 하이몰디브]", $content, 'HTML');
		//고객용
		getSendMail($_POST[to_email].'|'.$_POST[to_name] , $_POST[admin_add_email].'|'.$_POST[from_name], $_POST[to_name]."님의 ".$_POST[val3]." 여행계약서 입니다. [주식회사 트레비아, 하이몰디브]", $content, 'HTML');

if ( $_POST[add_email] ) {
		getSendMail($_POST[add_email].'|'.$_POST[to_name] , $_POST[admin_add_email].'|'.$_POST[from_name], $_POST[to_name]."님의 ".$_POST[val3]." 여행계약서 입니다. [주식회사 트레비아, 하이몰디브]", $content, 'HTML');
}
//$_POST[from_email] = "namsm05@hotmail.com";
//$_POST[from_name] = "남성민";

		//관리자용1
		getSendMail($_POST[from_email].'|'.$_POST[from_name] , $_POST[admin_add_email].'|'.$_POST[from_name], $_POST[to_name]."님의 ".$_POST[val3]." 여행계약서 입니다. [주식회사 트레비아, 하이몰디브]", $content, 'HTML');

		//관리자용2
		getSendMail($_POST[admin_add_email].'|'.$_POST[from_name] , $_POST[admin_add_email].'|'.$_POST[from_name], $_POST[to_name]."님의 ".$_POST[val3]." 여행계약서 입니다. [주식회사 트레비아, 하이몰디브]", $content, 'HTML');

//echo $_POST[to_email] . " - " . $_POST[to_name] . " - " . $_POST[add_email] . " - " . $_POST[from_email] . " - " . $_POST[admin_add_email];

	$qrys = mysql_query("select * from rb_contract_data where puid='".$_POST['puid']."' and ip like '%.%' ");
    $exist = mysql_num_rows($qrys);
	if( $exist > 0 )
		$category = iconv('utf-8','euc-kr',"계약서재발송");
	else
		$category = iconv('utf-8','euc-kr',"계약서발송");
?>
<iframe name='smsframe' width='0' height='0' frameborder='0' scrolling='no'></iframe>
<form name="smsForm" action="/modules/bbsext/theme/_pc/list02/sms_ajax.php" method="post" onsubmit="return preSubmit()" >
<input type="hidden" name="xms" id="xms" value="<?=$_POST[xms]?>" />
<input type="hidden" name="myuid" id="myuid" value="<?=$_POST[myuid]?>" />
<input type="hidden" name="memberuid" id="memberuid" value="<?=$_POST[memberuid]?>" />
<input type="hidden" name="uid" id="uid" value="<?=$_POST[uid]?>" />
<input type="hidden" name="r" id="r" value="<?=$_POST[r]?>" />
<input type="hidden" name="m" id="m" value="<?=$_POST[m]?>" />
<input type="hidden" name="bid" id="bid" value="<?=$_POST[bid]?>" />
<input type="hidden" name="receiver" id="receiver" value="<?=$_POST[receiver]?>" />
<input type="hidden" name="smsText" id="smsText" value="<?=urlencode(iconv('utf-8','euc-kr',"트레비아에서 여행계약서를 이메일과 모바일로 전송하였습니다.http://trevia.co.kr/modules/bbsext/theme/_pc/list02/mobile.php?uid=".$uid."&puid=".$puid))?>" />
<input type="hidden" name="custom_number" value="<?=$_POST[custom_number]?>" class="addInput" /><br />
<input type="hidden" value="<?=$_POST[send_tel_0]?>" name="send_tel_0" style="width:90px;border:0; border-bottom-style:none;" />
<input type="hidden" value="<?=$_POST[send_tel_1]?>" name="send_tel_1" style="width:90px;border:0; border-bottom-style:none;" />
<input type="checkbox" name="check_tel_0" checked style="display:none" />
<input type="checkbox" name="check_tel_1" checked style="display:none"/>
...
<script>
	...
	parent.opener.location.href="/modules/bbsext/theme/_pc/list02/view_process.php?category=<?=urlencode($category)?>&r=<?=$_POST[r]?>&m=<?=$_POST[m]?>&bid=<?=$_POST[bid]?>&uid=<?=$_POST[puid]?>";
	//self.close();
