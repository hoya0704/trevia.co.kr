...
if($bbsid == "resort")
{
	$content = $content . "[bc_separator]";
	$content .= $content2 . "[bc_separator]";
	$content .= $content3 . "[bc_separator]";
	//$content .= $content4 . "[bc_separator]";

	$upload	= $upfiles;// . $upfiles2 . $upfiles3 . $upfiles4;
}
else
{
...
//$adddata	= is_array($adddata) ? serialize($adddata) : trim($adddata);
...
if ($num_upfile || $num_photo)
{
	...
	$fserverurl = $fserver ? $d['upload']['ftp_urlpath'] : $g['url_root'].'/files/';
	$incPhoto	= '';
	$upload		= $uid ? $upload : '';
	$saveDir	= $g['path_file'];
...
if ($uid)
{
	...
	{	
		...
		if($bbsid == "resort")
		{
			$infoPrev = getDbData($table[$m.'data'],"uid=".$uid,"subject");
			getDbUpdate($table[$m.'data'],"category='".trim($subject)."'","category='".$infoPrev['subject']."'");
		}
...
// 1:1 상담 문의 메일전송 코드
if ($bbsid == "counselling")
{
	$adddata = unserialize($adddata);
	$img_module_skin = "/modules/bbs/theme/_pc/counselling/image";
	$mainContent = "";

	// 메일 제목에 헤더 표기 위한 사이트 코드 판독
	if($_POST['r'] == "maldives") $subject_header = "하이몰디브";
	else $subject_header = "트레비아";

	$mail_to = "info@trevia.co.kr";
	//$mail_to = "ftcaliburn@gmail.com";
	$headers = "from: " . $name . " <" . urldecode($adddata['email']) . "> \r\n";
	$headers .= "Content-Type: text/html; charset=EUC-KR \r\n";
	$subject =  "[".$subject_header."] " . $name . "님께서 요청하신 상담내용입니다.";

	$mainContent .= "<a href='http://trevia.co.kr/?r=maldives&m=bbs&uid=" . $NOWUID . "' style='font-size: 20px; color: #0000ff;'><strong>상담내역 게시판에서 확인하기</strong></a>";
	$mainContent .= "<br /><br /><br />";
	$mainContent .= "<h1>필수정보</h1><br />";
	$mainContent .= "<strong>고객명</strong> : " . $name . "<br />";
	$mainContent .= "<strong>핸드폰</strong> : " . urldecode($adddata['cellphone1']) . "-" . urldecode($adddata['cellphone2']) . "-" . urldecode($adddata['cellphone3']) . "<br />";
	$mainContent .= "<strong>이메일</strong> : " . urldecode($adddata['email']) . "<br />";
	$mainContent .= "<strong>여행지역</strong> : " . urldecode($adddata['region']) . "<br />";
	$mainContent .= "<strong>여행종류</strong> : " . urldecode($adddata['kind']) . "<br />";
	$mainContent .= "<strong>총 여행인원</strong> : " . urldecode($adddata['person_count']) . "<br />";
	$mainContent .= "<strong>출발날짜</strong> : " . urldecode($adddata['start_date']) . "<br />";
	$mainContent .= "<strong>예정</strong> : " . urldecode($adddata['scheduled']) . "<br />";
	$mainContent .= "<br /><br /><br />" . "<br />";
	$mainContent .= "<h1>부가정보</h1>" . "<br />";
	$mainContent .= "<strong>예산</strong> : " . urldecode($adddata['budget']) . "만원<br />";
	$mainContent .= "<strong>상담 희망시간</strong> : " . urldecode($adddata['wish_time1']) . " ~ " . urldecode($adddata['wish_time2']) . "<br />";
	$mainContent .= "<strong>희망항공사</strong> : " . urldecode($adddata['airline']) . "<br />";
	$mainContent .= "<strong>팩스번호</strong> : " . urldecode($adddata['fax1']) . "-" . urldecode($adddata['fax2']) . "-" . urldecode($adddata['fax3']) . "<br />";
	$mainContent .= "<strong>희망 리조트</strong> : " . urldecode($adddata['wish_resort1']) . ", " . urldecode($adddata['wish_resort2']) . ", " . urldecode($adddata['wish_resort3']) . "<br />";
	$mainContent .= "<strong>기타 요청사항</strong> : " . $content . "<br />";


	$mail_to = iconv("utf-8", "euc-kr", $mail_to);
	$subject = iconv("utf-8", "euc-kr", $subject);
	$mainContent = iconv("utf-8", "euc-kr", $mainContent);
	$headers = iconv("utf-8", "euc-kr", $headers);

	$mail_result = mail($mail_to,$subject,$mainContent,$headers);

	$sms = "[".$subject_header."]요청하신 여행견적이 접수되었습니다. 24시간(영업일) 내 전화상담 및 여행견적서로 발송되어 집니다.";
	$title = "견적요청(".$my['name'].")";
	if($my['tel2']) $cpnumber = $my['tel2'];
	else  $cpnumber = urldecode($adddata['cellphone1']) . urldecode($adddata['cellphone2']) . urldecode($adddata['cellphone3']);
	getSendSMS("_sms_","sms",$cpnumber,$title,$sms,"mms");
}
...
else if ($backtype == 'view')
{
	...
	if ($bbsid == "counselling")
	{
		getLink('reload','parent.','신청이 완료되었습니다.','');
	}
	else
	{
