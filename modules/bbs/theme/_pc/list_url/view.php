<?php
	...
	## 이미지 리스트 불러오기
	preg_match_all("/<img[^>]*src=['\"]?([^>'\"]+)['\"]?[^>]*>/", $R['content'], $aryImageList);
	$aryImageList = $aryImageList[1];
?>
<div id="bbsview">
	...
	<div class="viewbox">
		...
		<div class="linkurl">
			...
			URL : [<?php echo $adddata['linkheader']?>] <a href="<?php echo urldecode($adddata['linkurl'])?>" target="_blank"><?php echo urldecode($adddata['linkurl'])?></a>
		</div>
	...
	<div class="bottom">	
		<?php if($my['admin'] || $R['mbruid'] == $my['memberuid'] ):?>
