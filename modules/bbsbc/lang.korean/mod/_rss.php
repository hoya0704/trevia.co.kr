...
<rss version='2.0' xmlns:dc='http://purl.org/dc/elements/1.1/'>
	<channel>
		...
		<link><?php echo $g['s']?>/?r=<?php echo $r?>&amp;m=<?php echo $m?><?php if($B['uid']):?>&amp;bid=<?php echo $B['id']?><?php endif?></link> 
		<dc:language><?php echo $_HS['lang']?></dc:language>
<?php foreach($RCD as $R):?>
		<item>
			<title><?php echo htmlspecialchars($R['subject'])?></title>
			<description><![CDATA[<?php echo getContents($R['content'],$R['html'],'')?>]]></description>
			<link><?php echo $g['s']?>/?r=<?php echo $r?>&amp;m=<?php echo $m?>&amp;bid=<?php echo $R['bbsid']?>&amp;uid=<?php echo $R['uid']?></link>
			...
			<?php endfor?>
			<?php endif?>
			<dc:date><?php echo getDateFormat($R['d_regis'],'Y.m.d H:i')?></dc:date>
...
<feed version="0.3"
	...
	<id><?php echo $g['s']?>/?r=<?php echo $r?>&amp;m=<?php echo $m?><?php if($B['uid']):?>&amp;bid=<?php echo $B['id']?><?php endif?></id> 
...
<?php foreach($RCD as $R):?>
	<entry>
		...
		<id><?php echo $g['s']?>/?r=<?php echo $r?>&amp;m=<?php echo $m?>&amp;bid=<?php echo $R['bbsid']?>&amp;uid=<?php echo $R['uid']?></id>
		<created><?php echo getDateFormat($R['d_regis'],'Y.m.d H:i')?></created>
		<modified><?php echo getDateFormat($R['d_modify'],'Y.m.d H:i')?></modified>
